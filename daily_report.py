...
def get_analysis(ticker, name, market, price, ma20, ma60, rsi, data_date):
    ...
    prompt = f"""당신은 10년 경력의 주식 애널리스트입니다. 한국어로 핵심만 담은 투자 판단 리포트를 작성하세요.
...
작성 원칙:
- 핵심만, 군더더기 없이
- 투자자가 "오늘 살까 말까"를 바로 결정할 수 있도록
- 반드시 4개 항목 모두 완성 후 [END] 표시

1. 📰 최근 뉴스 & 여론
이 종목 관련 가장 최근 뉴스와 시장 여론을 2문장으로만 요약. 구체적 사실 위주.
...
결론: "[매수/매도/관망]" — 한 줄 핵심 요약 (구체적 행동과 가격 포함)
